...
export default class FileReportBtn extends Component {
   ...
   */
  @computed('reports.[]', 'args.file.canGenerateReport')
  get prevReports() {
  ...
  @computed('latestReport.{isGenerating,isGenerated,isNotGenerated}', 'args.file.{canGenerateReport,isStaticDone,isDynamicStatusInProgress,isRunningApiScan}', 'args.isSecurityDashboard', 'isReportGenerating')
  get enableBtn() {
  ...
  @computed('args.file.canGenerateReport', 'latestReport.{isGenerating,isGenerated}', 'isReportGenerating')
  get btnLabel() {
    let btnLabel = this.intl.t('report.generate');
    if (this.latestReport.isGenerating || this.isReportGenerating) {
      btnLabel = this.intl.t('report.generating');
    } else if (this.args.file.canGenerateReport) {
      btnLabel = this.intl.t('report.generate');
    } else if (this.latestReport.isGenerated) {
      btnLabel = this.intl.t('report.generated')
    }
    return btnLabel
  }
  ...
  @task(function* () {
    ...
    try {
      ...
      this.notify.error(parseError(error), this.intl.t('report.genreateError'));
    }
  ...
  @task(function* (type, reportId) {
    ...
    try {
      ...
      if (report && report.url) {
        ...
        this.notify.error(this.intl.t('report.downloadUrlNotFound'))
      }
    } catch {
